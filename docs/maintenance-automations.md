...
| 🧪 knowledge-adapters weekly chaos-all validation | `knowledge-adapters-weekly-chaos-all-validation` | Run exhaustive scheduled validation for `knowledge-adapters` and report the result. | Weekly. | `knowledge-adapters`. | Report-only validation. | Refresh from current `origin/main` only through the safe local refresh rule below, run `make chaos-all`, and report the tested commit SHA. If the checkout cannot be refreshed safely, skip validation and report the blocker instead of changing branches, rebasing, stashing, resetting, or cleaning local state. Do not modify files, open PRs, or run live-service or credential-dependent checks. |
...
- Audit prompt/config alignment from the active local automation configuration
  files, not only from this reference inventory. Use the inventory to orient
  the review, then inspect the owning `automation.toml` prompt/config and any
  companion config files before classifying or correcting an automation.
