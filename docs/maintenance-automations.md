...
| 🗜️ memory-compaction | `memory-compaction` | Inspect `~/.codex/automations/*/memory.md`, report memories over 25 KiB, and propose compact replacements that preserve durable state. | Weekly. | Local Codex automation memory files. | Report-only proposal. | Do not mutate `memory.md` automatically. |
