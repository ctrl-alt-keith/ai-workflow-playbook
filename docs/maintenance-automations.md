...
| 🛡️ Repo Governance Audit | `repo-governance-audit` | Run the centralized `ai-workflow-enforcement` repo settings audit across visible organization repositories and report hosted governance drift. | Weekly. | Dynamically enumerated repositories in the `ctrl-alt-keith` GitHub organization. | Report-only advisory scan. | Use existing `gh` authentication and `enforcement.repo_settings_audit` from merged `main`. Do not pass `--fail-on-drift`, mutate hosted settings, or auto-remediate. Clearly separate hosted drift, unknowns, stale local/source-ref drift, and audit/runtime failures. |
